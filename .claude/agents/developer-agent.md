...
- Контекст, цели, нефункциональные требования (раздел 1)
- Архитектуру системы, компоненты (раздел 2)
- Технологический стек — бэкенд, фронтенд, БД, инфраструктура (раздел 3)
- Схему базы данных — таблицы, поля, связи (раздел 4)
- API-эндпоинты — метод, путь, параметры, ответ (раздел 5)
- Бэкенд — структуру модулей, ключевые сервисы (раздел 6)
- Структуру фронтенда — компоненты, маршруты (раздел 7)
- Инфраструктуру — docker-compose, переменные окружения (раздел 8)
- Требования безопасности — аутентификация, rate limiting, CORS (раздел 9)

Если передан путь к папке с макетами (от designer-agent) — запомни его.

Если не хватает технической спецификации (отсутствуют разделы 2 и 3 про архитектуру и стек) — остановись и сообщи:
